...
**目标** = 识别出来的**指定宠物**（通过 YOLO 模型检测）

- 模型：`models/trained/luoke_pet.pt`
- 类别：奇丽草群组（class_id = 0）
- 检测结果包含：边界框 (x1, y1, x2, y2)、置信度、中心点
...
> 持续在检测到的**目标**周围绘制边界框，不闪烁、不消失。

没有实时打框，后续的瞄准、投掷都无法执行。

```
检测 → 打框 → 判断捕捉状态 → 瞄准 → 投掷
  ↑                                              │
  │______________________ 返回搜索 _______________│
```
...
- **实时打框**（基础）: 持续在检测到的**目标**周围绘制边界框
- **自动瞄准**: 鼠标移动到**目标**中心
- **投掷执行**: 向**目标**投掷精灵球（mouse_down → 微调 → mouse_up）
- **捕捉开始**: 检测到捕捉状态（精灵球界面）+ 有目标

### 目标生命周期

```
1. 检测到目标 → 开启实时打框
2. 有目标 + 进入捕捉状态 → 自动瞄准 + 投掷
3. 投掷完成 → 等待下一次捕捉
4. 目标消失 → 停止打框，返回搜索
```
...
def test_aim_and_throw_realtime():
    ...
    while running:
        ...
        # 3. YOLO 检测 → 获取目标
        detections = detector.detect(frame)  # 返回目标列表
        
        # 4. 实时打框（基础）- 围绕目标绘制边界框
        for target in detections:
            overlay.draw_box(target)  # 目标中心 + 边界框
        
        # 5. 判断捕捉状态
        if is_capture_mode(frame) and detections:
            target = detections[0]  # 选择最近的目标
            
            # 6. 自动瞄准 → 瞄准目标中心
            aim_and_throw.aim_only(target)
            
            # 7. 投掷执行 → 向目标投掷
            aim_and_throw.aim_and_throw(target)
            # 5. 自动瞄准
