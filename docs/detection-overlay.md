...
**死区过滤**：bbox 中心移动小于 `deadzone_px`（默认 3px）时不更新位置，
消除静止目标的微抖动。
...
| 状态 | 行为 | 推荐参数 |
|------|------|----------|
| Candidate | 记录坐标，绘制黄色细线框（让用户知道"正在确认中"） | 确认帧数 N=4 |
...
- 只有连续 `confirm_frames` 帧（默认 4）都成功匹配到，才转为 Active 状态开始绘框
- 中途丢失则重置计数，直接销毁
